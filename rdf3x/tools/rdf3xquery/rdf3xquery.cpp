...
/*
unsigned dijkstra(unsigned int s, Database &db, unsigned int dist_id)
{
    map<unsigned, pair<unsigned,std::vector< pair<unsigned,unsigned> > > >g;
    map<unsigned int, unsigned int> d1; //distance
    map<unsigned int, unsigned int> c1; //state
    ...
    while(!q.empty())
    {
        ...
        if(c1[u]==2)
            continue;
        c1[u]=2; 
        for(unsigned i=0;i<(g[u].second).size();i++)
        {
            uint32_t v=(g[u].second)[i].second; //4byte 
            unsigned predicateBeforeObject=(g[u].second)[i].first;
            if(c1[v]==0) //new vertex found
            {
                d1[v]=d1[u]+1;
...
//-------------------------------------------------------
void dijkstra_implemenation(char* argv, string srcDest[2]) //srcDest[2] is an array of source destination pairs
{
    Database db;
       // Open the database   
    if (!db.open(argv,true)) {
    ...
    unsigned distanceFromSourceToDestination=0;
    distanceFromSourceToDestination=dijkstra(id, db, dist_id);
    ...
    db.close();  
    outputFile1.open("OutputFile.txt", ios::app|ios::out);           
    outputFile1<<distanceFromSourceToDestination<<"\t"<<mtime<<" msec";    
    outputFile1.close();
}
*/
