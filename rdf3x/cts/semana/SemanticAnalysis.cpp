...
//Jyoti --begin
void SemanticAnalysis::lookupNode( std::string& text, Type::ID type, unsigned subType, unsigned& id)
{
    lookup(dict,diffIndex,text ,type ,subType,id);
...
//Jyoti --Inside this function is the look-up that I am searching for.
{
   switch (element.type) {
   ...
   // Transform a subquery
{
   ...
   for (std::vector<SPARQLParser::Pattern>::const_iterator iter=group.patterns.begin(),limit=group.patterns.end();iter!=limit;++iter) {
      ...
      //Jyoti --begin
      cout<<"\n semantic node.subject="<<node.subject;
      cout<<"\n semantic node.predicate="<<node.predicate;
      cout<<"\n semantic node.object="<<node.object;
